...
const PendingAssignment = () => {
  ...
  const handleGiveMark = () => {
    Swal.fire({
      ...
      footer:
        '<p className="text-red-600">This is your assignment. <br/>So, You can only give mark the assignment which is not created by you.</p>',
    });
  };
  return (
    <div className="lg:mx-24 p-4 lg:p-12">
      <div className="overflow-x-auto">
        <table className="min-w-full text-sm">
          ...
          <tbody>
            {submitedAssignments.map((submitedAssignment) => (
              <tr
                key={submitedAssignment._id}
                className="border-b text-blue-800 border-opacity-20 border-gray-700 bg-gray-300"
              >
                ...
                <td className="p-3 text-right">
                  {submitedAssignment.userEmail === user.email ? (
                    <button
                      onClick={() => handleGiveMark(submitedAssignment._id)}
                      className="px-3 py-2 font-semibold rounded-md bg-violet-400 text-gray-900 hover:bg-violet-300 hover:-tracking-wide cursor-pointer"
                    >
                      My Assignment
                    </button>
                  ) : (
                    <Link
                      to={`/giveMarks/${submitedAssignment._id}`}
                      className="px-7 py-2 font-semibold rounded-md bg-violet-400 text-gray-900 hover:bg-violet-300 hover:-tracking-wide cursor-pointer"
                    >
                      Give Mark
                    </Link>
                  )}
