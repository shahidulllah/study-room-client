...
const Assignment = () => {
  ...
  const handleFilter = (e) => {
    ...
    if (e === "All") {
      ...
      const easyAssignment = assignments.filter(
        (a) => a.difficultyLevel === "Easy"
      );
      setDisplayAssign(easyAssignment);
    } else if (e === "Medium") {
      const mediumAssignment = assignments.filter(
        (a) => a.difficultyLevel === "Medium"
      );
      setDisplayAssign(mediumAssignment);
    } else if (e === "Hard") {
      const hardAssignment = assignments.filter(
        (a) => a.difficultyLevel === "Hard"
      );
  ...
  const handleDelete = (id, userEmail) => {
    if (userEmail !== user.email) {
      Swal.fire({
        ...
        footer:
          '<p className="text-red-600">This is not your assignment. <br/> You can only delete the assignment which is created by you.</p>',
      });
    } else {
      Swal.fire({
        title: "Are you sure?",
  ...
  return (
    <div className="p-4 lg:mx-24">
      <div className="flex flex-col items-center justify-center  pb-10 space-y-5">
        ...
        <p className="text-center lg:w-5/12 text-gray-500">
          Most interesting part to have the fresh mind by keeping our product.
          Doing assignment is the way to live and refresh your mind.
        </p>
      </div>
      {/* Filter */}
      <div className="flex justify-center">
        <div className="flex justify-center w-6/12 divide-x rounded bg-violet-400 text-gray-800 divide-gray-700">
          <button
            type="button"
            className="px-4 lg:px-8 lg:py-3 font-semibold text-lg flex items-center gap-3"
          >
            <PiUserList className="text-2xl hidden" /> Filtered Assignments
          </button>
          <button type="button" title="Toggle dropdown" className="p-3">
            <div className="dropdown">
              <div tabIndex={0} role="button" className=" bg-violet-400 m-1">
                <IoIosArrowDown className="text-2xl" />
              </div>
              <ul
                tabIndex={0}
                className="dropdown-content z-[1] menu p-2 bg-violet-300 rounded-lg w-32"
              >
      ...
      <div className="grid grid-cols-1 items-center md:grid-cols-2 lg:grid-cols-3 gap-7 mt-10 mb-12">
        {displayAssign.map((assignment) => (
          <div
            key={assignment._id}
            className="flex h-[590px] flex-col  rounded-2xl shadow-2xl border-2 "
          >
            <img
            ...
            <div className="flex flex-col justify-between p-6 space-y-8">
              <div className="space-y-2">
                <h2 className="text-3xl font-semibold tracking-wide text-purple-900 font-serif">
              ...
              <div className="space-y-1">
                <div className="flex items-center justify-between">
                  <div className="flex items-center">
                    ...
                    <span className="font-bold mr-2">Marks:</span>{" "}
                    <span className="font-extrabold">{assignment.marks}</span>
                  </div>
                  <div>
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center">
                    <FaRegStar className="mr-2"></FaRegStar>
                    <span className="font-bold mr-2">DiffiltyLevel:</span>{" "}
                  ...
                  <div>
                    <button
                      onClick={() =>
                        handleDelete(assignment._id, assignment.userEmail)
                      }
                      className="p-2 bg-gray-200 rounded-xl hover:bg-gray-300"
              ...
              <div className="">
                <Link to={`/viewAssignment/${assignment._id}`}>
                  ...
                  <button
                    type="button"
                    className="flex btn  items-center justify-center w-full p-3 font-extrabold tracking-wide rounded-md "
                  >
